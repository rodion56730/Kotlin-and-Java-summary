...
**`onReceive()`** — единственный метод для переопределения. Выполняется в главном потоке, должен завершиться максимально быстро (не более 5 секунд, иначе ANR). Тяжёлую работу нужно передавать в Service или WorkManager через `goAsync()` (возвращает `PendingResult`, позволяя завершить обработку асинхронно).
